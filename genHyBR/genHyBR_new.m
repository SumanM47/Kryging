...
% [x_out,output] = genHyBR_new(A,b,Q,R,options)
% 
% execute genHyBR method for given values of the parameter and get x and internal structures
% 
% Input:
%         A - matrix
%         b - vector
%         Q,R - covariance matrices
%         options - structures from genHyBR (see genHyBR_lsmr)
% 
% Output:
%         x_out - optimal x
%         output - structure of internal elements
%             output.U, output.V, output.B - matrices created by genGKB
%             output.f, output.Vf - vectors created internally, needed for evaluating the likelihood
% 

% default options for optional inputs
...
% modifying input R by the given parameter value
R = R/sigma_e;
...
% modifying b to account for non-zero mean
A1 = sum(A,2);
if mu~=0
    b = b - mu*A1;
end

% Computing beta1, initializing U
beta = normM(b,@(x)R\x);
U = (1 / beta)*b;
GKhandle = @genGKB;
B = []; V = [];
...
%else
% genGKB to produce the bidiagonalization at each iteration
...
if lambda == 0
  ...
  Vf = (V*f);
  x = Q*Vf; % this is mean deducted x, the output has mean added to it as x_out
else
	f = (B'*B + lambda*speye(size(B,2)))\(B'*vector);
        Vf =(V*f); 
        x = Q*Vf; % this is mean deducted x, the output has mean added to it as x_out
