...
function [x_out, delx, output] = genHyBR_plain(A, b, Q, R, options)
...
outcri1 = nargout>2 && grad;
outcri2 = nargout>1 && ~grad;
outputparams = outcri1 || outcri2;
if outputparams
...
for i = 1:maxiter+1 %Iteration (i=1) is just an initialization
  if grad
      [U,B,V,dBmu,dBsigma_e,dBrange,dVmu,dVsigma_e,dVrange,dUmu,dUsigma_e,dUrange] = feval(GKhandle, A, Q, R, U, B, V, dqrange, sigma_e, dBmu,dBsigma_e,dBrange,dVmu,dVsigma_e,dVrange,dUmu,dUsigma_e,dUrange,options);
  else
